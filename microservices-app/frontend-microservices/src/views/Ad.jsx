...
class Ad extends RoleAwareComponent {
  ...
  componentDidMount() {
    ...
    axios({
      method: 'get',
      url: url + 'advertisement/advertisement/cadEndUserAdd',
      headers: { "Authorization": AuthStr } ,       
    }).then((response) => {
      if (response.status === 200)
  ...
  submit(e) {
    ...
    if (ok && this.state.dodaoSlike) {
        ...
        axios({
          method: 'post',
          url: url + 'advertisement/advertisement/' + this.state.cjenovnikId,
          data: data,
          headers: { "Authorization": AuthStr }
        }).then((response) => {
          ...
          if (this.state.dodaoSlike !== false) {
            ...
            let reqURL =  url + 'advertisement/image/' + pom;
            axios({
              method: 'post',
              url: reqURL,
              data: this.lista,
            ...
            <Card>
              ...
              <CardBody hidden={this.state.possible}>
                ...
                <FormGroup>
                  ...
                  <input type="file" accept="image/png,image/jpeg" onChange={this.uploadMultipleFiles} multiple />
