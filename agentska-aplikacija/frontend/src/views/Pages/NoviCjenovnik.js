...
class NoviCjenovnik extends RoleAwareComponent {
...
edit(e){
  ...
  if(ok){
    ...
    axios({
      method: 'put',
      url: url + 'pricelist/'+this.state.editId,
      data: data,
      headers: { "Authorization": AuthStr },  
    }).then((response)=>{ 
      let temp = this.state.pricelists;
      temp = temp.filter(item=> item.id !== response.data.id);
...
submit(e){
  e.preventDefault();
        ...
        ok = this.popust30Validation(this.popust30) && ok;
